            ...
            >
              {question.options?.map((option) => (
                <div key={option.label} className="w-full max-w-[150px]">
                  ...
                  <Label
                    htmlFor={option.label}
                    className="flex h-full cursor-pointer flex-col items-center justify-center gap-2 rounded-md border-2 border-[#e0eede] bg-white p-4 text-lg transition-transform active:scale-[0.98] active:bg-[#5a8230] hover:bg-primary/10 peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/10 [&:has([data-state=checked])]:border-primary"
                  >
                    {option.imageUrl && (
                      <Image
                        src={option.imageUrl}
